...
void videoRead(const char *video_name, int cpuid)
{
	...
	while (1)
	{
		// cv::Mat img_src(cv::Size(video_probs.Video_height, video_probs.Video_width), CV_8UC1);
		cv::Mat img_src;
		// 如果读不到图片 或者 bReading 不在读取状态则跳出
		if (!video.read(img_src))
		{
			printf("read video stream failed! Maybe to the end!\n");
			video.release();
			break;
		}
		imagePool.emplace_back(img_src);
	}
	printf("VideoRead is over.\n");
}
...
void videoResize(int cpuid)
{
	...
	if (pthread_setaffinity_np(pthread_self(), sizeof(mask), &mask) < 0)
		std::cerr << "set thread affinity failed" << endl;
